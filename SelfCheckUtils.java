...
/**
 * This will essentially be a class of Static methods that you can use to test
 * anything in your own classes. I will be adding more features to this as time
 * goes on.
 * 
...
public class SelfCheckUtils {
	...
	/**
	 * Just for testing items in this class. Make sure your application does not use
	 * this main method for running your own applications!
	 * 
	...
	/**
	 * Runs a self-check and pretty-print the result of the check. Note that the
	 * colors may not show correctly in some terminals.
	 * 
	 * DO NOT USE THIS AS THE ACTUAL SELF-CHECK method! This will call self-check
	 * method of the item passed in, so if you call it from the self-check method
	 ...
	 * Test t = new new Test(...);
	 * 
	 * From outside the Test class: SelfCheckUtils.basicSelfCheckRunner(t);
	 * 
	 * From inside the Test class (e.g. from runSelfCheck):
	 * SelfCheckUtils.basicSelfCheckRunner(this);
	 * 
	 * @param item the item to be tested
	 * @return the colorized version of the run. Colors may not show in all
	 *         terminals
	 */
	...
	/**
	 * A basic checkComponents method that tests all componets that are provided as
	 * parameters.
	 * 
	 * Again, do not use this from the selfCheck method, but can be used from the
	 * runSelfCheck method to check all components in the class.
	 * 
	 * SelfCheckUtils.checkComponents(wheels, sensors, camera);
	 * 
	 * @param items A comma separated list of items - can be any object, but only
	 *              those that implement SelfCheckCapable will be tested. Supports
	 *              Arrays of SelfCheckCapable objects Supports Lists of
	 *              SelfCheckCapable objects
	...
	/**
	 * Super simple random check utility - simply calls a random function and
	 * returns true for half the cases, and false for half.
	 * 
	...
	/**
	 * Another check utility - simply calls a random function and returns true for
	 * half the cases, and false for half.
	 * 
	 * To call this, you can simply do a return SelfCheckUtils.randomCheck(0.25); //
	 * 25% chance of failure
	 * 
	 * @param failprobability - the probability of a failure - send as a value
	 *                        between 0 and 1. 1 means 100% probability of failure,
	...
	/**
	 * An internal method to "prettify" the test result If the test result is false,
	 * a red FAILED string is returned If the test result is true, a green check ok
	 * string is returned If terminal is not set, no colors are applied.
	 * 
	...
	private static String prettify(boolean result) {
		if (System.console() != null && System.getenv().get("TERM") != null) {
			return (result ? ANSI_GREEN + "check ok" + ANSI_RESET : ANSI_RED + "FAILED!" + ANSI_RESET);
		} else
			return (result ? "check ok" : "FAILED!");
	}
